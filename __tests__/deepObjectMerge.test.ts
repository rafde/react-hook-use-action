...
describe('deepMerge', () => {
	...
	test('merges nested changes', () => {
		...
		expect(result).toStrictEqual({
			user: { name: 'John', settings: source.user.settings },
		});
	});

	test('merge when there is one change', () => {
		const target = { user: { name: 'John', settings: { theme: 'dark' } } };
		const source = { user: { name: 'Jon', settings: { theme: 'dark' } } };
		const result = deepObjectMerge(target, source, compare);
		expect(result).toStrictEqual({
			user: {
				...source.user,
	...
	test('returns undefined for identical nested structures', () => {
		const target = { user: { name: 'John', settings: { theme: 'dark' } } };
		const source = { user: { name: 'John', settings: { theme: 'dark' } } };
		const result = deepObjectMerge(target, source, compare);
		expect(result).toBeUndefined();
	});
